...
# Список пар слов (кортежей) одинаковой длины из first_strings и second_strings
second_result = [(f, s) for f in first_strings for s in second_strings if len(f) == len(s)]

# Словарь, где ключом является строка, а значением - длина строки из объединённых списков, если длина чётная
