...
# Метод _str_, который возвращает строку в формате '<название>, <вес>, <категория>'.
...
class Shop:
    ...
    def add(self, *products):
        # Получаем уже существующие продукты в магазине
        existing_products = self.get_products()

        with open(self.__file_name, 'a') as file:
            for product in products:
                # Проверяем, есть ли продукт уже в списке по имени
...
print(p2)  # __str__ -> Spaghetti, 3.4, Groceries

s1.add(p1, p2, p3)

print(s1.get_products())  # Получаем все продукты из файла