...
def custom_write(file_name, strings):
    ...
    with open(file_name, 'w', encoding='utf-8') as file:
        for i, string in enumerate(strings, 1):  # Перебираем строки с номером начиная с 1
            position = file.tell()  # Получаем текущую позицию указателя в файле
            file.write(string + '\n')  # Записываем строку и перевод строки
            strings_positions[(i, position)] = string  # Сохраняем позицию и строку в словарь
