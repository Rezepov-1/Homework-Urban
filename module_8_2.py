...
def personal_sum(numbers):
    ...
    for item in numbers:
        try:
            # Пытаемся добавить item к result
            result += item
        except TypeError:
            # Если возникает TypeError, печатаем сообщение об ошибке
...
def calculate_average(numbers):
    try:
        # Проверка, что numbers является коллекцией (списком или кортежем)
        if not isinstance(numbers, (list, tuple)):
            raise TypeError

        total_sum, incorrect_data = personal_sum(numbers)
        count = len(numbers) - incorrect_data
        # Возвращаем среднее арифметическое, если есть корректные данные
